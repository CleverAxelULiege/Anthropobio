import { TABS } from "../../App";
import { useAppSettings } from "../../AppSettingsContext";
import styles from "./Nav.module.css";


export default function Nav({ tab, setTab }) {
    const { expertMode, setExpertMode, primateId, setPrimateId } = useAppSettings();

    return (
        <nav className={styles.nav}>
            <div className={styles.buttonContainer}>
                <a className={styles.button} onClick={(e) => { e.preventDefault(); setTab(TABS.home) }} href={`?tab=${TABS.home}`}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path d="M341.8 72.6C329.5 61.2 310.5 61.2 298.3 72.6L74.3 280.6C64.7 289.6 61.5 303.5 66.3 315.7C71.1 327.9 82.8 336 96 336L112 336L112 512C112 547.3 140.7 576 176 576L464 576C499.3 576 528 547.3 528 512L528 336L544 336C557.2 336 569 327.9 573.8 315.7C578.6 303.5 575.4 289.5 565.8 280.6L341.8 72.6zM304 384L336 384C362.5 384 384 405.5 384 432L384 528L256 528L256 432C256 405.5 277.5 384 304 384z" /></svg>
                    Retour à l'accueil
                </a>
                <a className={styles.button} onClick={(e) => { e.preventDefault(); setTab(TABS.about) }} href={`?tab=${TABS.about}`}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path d="M320 576C461.4 576 576 461.4 576 320C576 178.6 461.4 64 320 64C178.6 64 64 178.6 64 320C64 461.4 178.6 576 320 576zM320 240C302.3 240 288 254.3 288 272C288 285.3 277.3 296 264 296C250.7 296 240 285.3 240 272C240 227.8 275.8 192 320 192C364.2 192 400 227.8 400 272C400 319.2 364 339.2 344 346.5L344 350.3C344 363.6 333.3 374.3 320 374.3C306.7 374.3 296 363.6 296 350.3L296 342.2C296 321.7 310.8 307 326.1 302C332.5 299.9 339.3 296.5 344.3 291.7C348.6 287.5 352 281.7 352 272.1C352 254.4 337.7 240.1 320 240.1zM288 432C288 414.3 302.3 400 320 400C337.7 400 352 414.3 352 432C352 449.7 337.7 464 320 464C302.3 464 288 449.7 288 432z" /></svg>
                    A propos
                </a>
                <button id="expert_mode_button" onClick={() => { setExpertMode(!expertMode) }} className={styles.button} style={{ marginLeft: "auto", minWidth: "350px" }}>
                    {!expertMode && <><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path d="M434.8 54.1C446.7 62.7 451.1 78.3 445.7 91.9L367.3 288L512 288C525.5 288 537.5 296.4 542.1 309.1C546.7 321.8 542.8 336 532.5 344.6L244.5 584.6C233.2 594 217.1 594.5 205.2 585.9C193.3 577.3 188.9 561.7 194.3 548.1L272.7 352L128 352C114.5 352 102.5 343.6 97.9 330.9C93.3 318.2 97.2 304 107.5 295.4L395.5 55.4C406.8 46 422.9 45.5 434.8 54.1z" /></svg> Passer en mode "Expert"</>}
                    {expertMode && <><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path d="M224 208C224 128.5 288.5 64 368 64C376.8 64 384 71.2 384 80L384 232.2C399 226.9 415.2 224 432 224C511.5 224 576 288.5 576 368C576 376.8 568.8 384 560 384L407.8 384C413.1 399 416 415.2 416 432C416 511.5 351.5 576 272 576C263.2 576 256 568.8 256 560L256 407.8C241 413.1 224.8 416 208 416C128.5 416 64 351.5 64 272C64 263.2 71.2 256 80 256L232.2 256C226.9 241 224 224.8 224 208zM320 352C337.7 352 352 337.7 352 320C352 302.3 337.7 288 320 288C302.3 288 288 302.3 288 320C288 337.7 302.3 352 320 352z" /></svg> Passer en mode "Light" </>}
                </button>

                {
                    // tab != TABS.learning &&
                    // <a className={styles.button} onClick={(e) => {e.preventDefault(); setTab(TABS.learning)}} href={`?tab=${TABS.learning}`}>
                    //     <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path d="M480 272C480 317.9 465.1 360.3 440 394.7L566.6 521.4C579.1 533.9 579.1 554.2 566.6 566.7C554.1 579.2 533.8 579.2 521.3 566.7L394.7 440C360.3 465.1 317.9 480 272 480C157.1 480 64 386.9 64 272C64 157.1 157.1 64 272 64C386.9 64 480 157.1 480 272zM272 416C351.5 416 416 351.5 416 272C416 192.5 351.5 128 272 128C192.5 128 128 192.5 128 272C128 351.5 192.5 416 272 416z" /></svg>
                    //     Découverte
                    // </a>
                }
                {
                    // tab != TABS.observation &&
                    // <a className={styles.button} onClick={(e) => {e.preventDefault(); setTab(TABS.observation)}}  href={`?tab=${TABS.observation}`}>
                    //     <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path d="M135.4 317.6C130.6 309.9 128 301.1 128 292L128 256C128 229.5 149.5 208 176 208L192 208L192 128C192 101.5 213.5 80 240 80C257.3 80 272.4 89.1 280.9 102.8C285.2 80.7 304.7 64 328 64C351.4 64 370.9 80.8 375.1 102.9C382.4 98.5 390.9 96 400 96C422.1 96 440.8 111 446.3 131.4C451.8 129.2 457.7 128 464 128C490.5 128 512 149.5 512 176L512 272.9C512 282.8 509.7 292.6 505.2 301.5L465.6 380.6C454.8 402.3 432.6 416 408.4 416L224 416C207.5 416 192.2 407.6 183.4 393.6L135.4 317.6zM160 544L160 496C160 478.3 174.3 464 192 464L448 464C465.7 464 480 478.3 480 496L480 544C480 561.7 465.7 576 448 576L192 576C174.3 576 160 561.7 160 544z" /></svg>
                    //     Manipulation
                    // </a>
                }
            </div>
        </nav>
    );
}
